...
public class Application extends com.atomgraph.core.Application {
    ...
    public Application(@Context ServletConfig servletConfig) {
        this(
                servletConfig.getServletContext().getInitParameter(A.dataset.getURI()) != null
                        ? getDataset(servletConfig.getServletContext().getInitParameter(A.dataset.getURI()), null)
                        : null,
                servletConfig.getServletContext().getInitParameter(SD.endpoint.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(SD.endpoint.getURI())
                        : null,
                servletConfig.getServletContext().getInitParameter(A.graphStore.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(A.graphStore.getURI())
                        : null,
                servletConfig.getServletContext().getInitParameter(A.quadStore.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(A.quadStore.getURI())
                        : null,
                servletConfig.getServletContext().getInitParameter(A.authUser.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(A.authUser.getURI())
                        : null,
                servletConfig.getServletContext().getInitParameter(A.authPwd.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(A.authPwd.getURI())
                        : null,
                new MediaTypes(), getClient(new ClientConfig()),
                servletConfig.getServletContext().getInitParameter(A.maxGetRequestSize.getURI()) != null
                        ? Integer.parseInt(
                                servletConfig.getServletContext().getInitParameter(A.maxGetRequestSize.getURI()))
                        : null,
                servletConfig.getServletContext().getInitParameter(A.cacheModelLoads.getURI()) != null ? Boolean
                        .parseBoolean(servletConfig.getServletContext().getInitParameter(A.cacheModelLoads.getURI()))
                        : false,
                servletConfig.getServletContext().getInitParameter(A.preemptiveAuth.getURI()) != null ? Boolean
                        .parseBoolean(servletConfig.getServletContext().getInitParameter(A.preemptiveAuth.getURI()))
                        : false,
                new LocationMapper(
                        servletConfig.getServletContext().getInitParameter(AP.locationMapping.getURI()) != null
                                ? servletConfig.getServletContext().getInitParameter(AP.locationMapping.getURI())
                                : null),
                servletConfig.getServletContext().getInitParameter(LDT.ontologyProperty.getURI()) != null
                        ? servletConfig.getServletContext().getInitParameter(LDT.ontologyProperty.getURI())
                        : null,
                servletConfig.getServletContext().getInitParameter(AP.cacheSitemap.getURI()) != null
                        ? Boolean.valueOf(servletConfig.getServletContext().getInitParameter(AP.cacheSitemap.getURI()))
                        : true);
    }

    public Application(final Dataset dataset, final String endpointURI, final String graphStoreURI,
            final String quadStoreURI,
        ...
        if (ontologyURI == null) {
            if (log.isErrorEnabled())
                log.error("Sitemap ontology URI (" + LDT.ontologyProperty.getURI() + ") not configured");
            throw new ConfigurationException(LDT.ontologyProperty);
        }
        ...
        DataManager dataManager = new DataManagerImpl(locationMapper, new HashMap<>(),
                LinkedDataClient.create(client, mediaTypes), true, preemptiveAuth);
        OntDocumentManager.getInstance().setFileManager((FileManager) dataManager);
        if (log.isDebugEnabled())
            log.debug("OntDocumentManager.getInstance().getFileManager(): {}",
        ...
        this.ontology = new OntologyLoader(OntDocumentManager.getInstance(), ontologyURI, OntModelSpec.OWL_MEM_RDFS_INF,
                true).getOntology();
    }
    ...
    public void init() {
        ...
        // if (log.isTraceEnabled()) log.trace("Application.init() with Classes: {} and
        // Singletons: {}", classes, singletons);
    }
